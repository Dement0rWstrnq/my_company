...
def get_workers(db: Session = Depends(db_session)) -> list[dto.Worker]:
...
def post_worker(
    request_body: dto.Worker.New, db: Session = Depends(db_session)
) -> dto.Worker:
...
@router.get(
    "/workers/{worker_id}", response_model=dto.Worker, status_code=status.HTTP_200_OK
)
def get_worker_id(worker_id: UUID, db: Session = Depends(db_session)) -> dto.Worker:
...
def put_position_id(
    worker_id: UUID, request_body: dto.Worker.Update, db: Session = Depends(db_session)
) -> dto.Worker.Update:
    result = select(tables.Worker).where(tables.Worker.id == worker_id)
    result = db.scalars(result).first()
...
def del_position_id(worker_id: UUID, db: Session = Depends(db_session)) -> dto.Worker:
    result = select(tables.Worker).where(tables.Worker.id == worker_id)
    result = db.scalars(result).first()
    if result:
        db.delete(result)
