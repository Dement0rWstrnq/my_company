...
@router.get("/departments", response_model=list[dto.Department], status_code=status.HTTP_200_OK)
def get_departments(
...
def create_department(
        request_body: dto.Department.New,
        db: Session = Depends(db_session)
) -> dto.Department:
    result = tables.Department(
...
@router.get("/departments/{department_id}", response_model=dto.Department, status_code=status.HTTP_200_OK)
