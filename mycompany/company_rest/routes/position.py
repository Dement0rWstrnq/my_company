...
@router.get(
    "/positions", response_model=list[dto.Position], status_code=status.HTTP_200_OK
)
def get_positions(db: Session = Depends(db_session)) -> list[dto.Position]:
...
def create_position(
    request_body: dto.Position.New, db: Session = Depends(db_session)
) -> dto.Position:
    result = tables.Position(name=request_body.name)
    db.add(result)
    db.commit()
    db.refresh(result)
    return result
...
@router.get(
    ...
    response_model=dto.Position,
    status_code=status.HTTP_200_OK,
)
def get_position_id(
    position_id: UUID, db: Session = Depends(db_session)
) -> dto.Position:
